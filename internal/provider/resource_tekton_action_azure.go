...
// Identical to the AWS action model — the per-cloud difference lives entirely
// in the provider block and the generated setup-credentials step.
...
func (r *TektonActionAzureResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description: "Manages a Tekton Task and StepAction for Azure-based workflows. " +
			"This resource prepends a setup-credentials step that runs `az login` using the " +
			"credentials configured in the provider's azure block, writing the resulting token " +
			"cache to /workspace/.azure. Subsequent steps receive AZURE_CONFIG_DIR and can call " +
			"`az` directly without handling credentials themselves.",
		Attributes: map[string]schema.Attribute{
			...
			"steps": schema.ListNestedAttribute{
				Description: "List of steps for the Tekton Task. Each step's image should include the Azure CLI " +
					"if the script calls `az`.",
				Required: true,
				NestedObject: schema.NestedAttributeObject{
					Attributes: map[string]schema.Attribute{
						...
						"image": schema.StringAttribute{
							Description: "Container image for the step (should include Azure CLI)",
							Required:    true,
						},
			...
			"step_action_name": schema.StringAttribute{
				Description: "Generated StepAction name for Azure credential setup (computed from hash). " +
					"This StepAction automatically configures Azure access for the workflow steps.",
				Computed: true,
			},
		},
	}
...
func (r *TektonActionAzureResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	//
	// Azure config is likewise validated lazily during CRUD, not here, so that
	// terraform validate succeeds without Azure credentials.
...
// Mirrors the AWS and Kubernetes variants — no cached state, thread-safe.
...
// resolveAzure pulls environment/resource metadata out of the model and
// validates the provider's azure block. Shared by Create and Update, which
// otherwise duplicate this block verbatim.
func (r *TektonActionAzureResource) resolveAzure(ctx context.Context, plan TektonActionAzureResourceModel) (*tekton.ResourceMetadata, *azure.AzureAuthConfig, diag.Diagnostics) {
	var diags diag.Diagnostics

	var facetsEnv tekton.FacetsEnvironmentModel
	diags.Append(plan.FacetsEnvironment.As(ctx, &facetsEnv, basetypes.ObjectAsOptions{})...)
	if diags.HasError() {
		return nil, nil, diags
	}

	var facetsRes tekton.FacetsResourceModel
	diags.Append(plan.FacetsResource.As(ctx, &facetsRes, basetypes.ObjectAsOptions{})...)
	if diags.HasError() {
		return nil, nil, diags
	}
	...
	azureConfig, err := azure.GetAzureConfig(ctx, &azure.ProviderModel{Azure: r.providerData.Azure})
	if err != nil {
		diags.AddError("Azure Configuration Error", err.Error())
		return nil, nil, diags
	}

	return metadata, azureConfig, diags
}
...
func (r *TektonActionAzureResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	metadata, azureConfig, diags := r.resolveAzure(ctx, plan)
	...
	task := r.buildAzureTask(ctx, plan, azureConfig, metadata.LabelsAsInterface())
...
// createResources creates the StepAction and Task in cluster. Mirrors the AWS variant,
// including the StepAction rollback when Task creation fails (issue #10).
func (r *TektonActionAzureResource) createResources(ctx context.Context, operations *tekton.ResourceOperations, stepAction, task *unstructured.Unstructured) diag.Diagnostics {
	...
	if err := operations.CreateResource(ctx, task, "tekton.dev", "v1beta1", "tasks"); err != nil {
		// Roll back the StepAction so it doesn't orphan in cluster.
		// DeleteResource is idempotent on NotFound (fix #11), so this is safe
...
// readResourceState performs the cluster-side existence check. Mirrors the AWS
// variant: NotFound is classified via apierrors.IsNotFound so only genuine
// deletions drop state; transient errors retain it; asymmetric drift warns.
...
func (r *TektonActionAzureResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Use state values for computed fields — they are unknown in the plan.
...
// updateResources updates the Task and StepAction in cluster, Task first.
// Same ordering rationale as the AWS variant: a failed Task update leaves the
// cluster coherent, and a failed StepAction update still resolves via the
// Task's immutable ref.name until the operator re-runs apply.
...
// deleteResources attempts to delete both objects best-effort, aggregating
// errors. Combined with idempotent DeleteResource, destroy retries are safe.
...
//
// User steps receive AZURE_CONFIG_DIR (pointing at the token cache the
// setup-credentials step populated) plus the subscription/tenant/client IDs, so
// both `az` and SDK-based tooling work without the step handling credentials.
// The client secret is deliberately NOT propagated past setup-credentials.
func (r *TektonActionAzureResource) buildAzureTask(ctx context.Context, plan TektonActionAzureResourceModel, azureConfig *azure.AzureAuthConfig, labels map[string]interface{}) *unstructured.Unstructured {
	...
	for _, step := range steps {
		...
		tekton.AddEnvVar(tektonStep, "AZURE_SUBSCRIPTION_ID", azureConfig.SubscriptionID)
		tekton.AddEnvVar(tektonStep, "AZURE_TENANT_ID", azureConfig.TenantID)
		tekton.AddEnvVar(tektonStep, "AZURE_CLIENT_ID", azureConfig.ClientID)
