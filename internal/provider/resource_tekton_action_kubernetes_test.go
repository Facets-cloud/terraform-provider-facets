...
func TestGenerateResourceNames(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			names2 := tekton.GenerateNames(tt.resourceName, tt.envName, tt.displayName)
			if taskName != names2.TaskName {
				t.Errorf("taskName not deterministic: %s != %s", taskName, names2.TaskName)
			}
			if stepActionName != names2.StepActionName {
				t.Errorf("stepActionName not deterministic: %s != %s", stepActionName, names2.StepActionName)
			}
		})
	}
}
...
func TestBuildLabels(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			metadata := &tekton.ResourceMetadata{
				DisplayName:   tt.displayName,
				ResourceName:  tt.resourceName,
				ResourceKind:  tt.resourceKind,
				EnvUniqueName: tt.envUniqueName,
				ClusterID:     tt.clusterID,
				IsCloudAction: tt.cloudAction,
				CustomLabels:  tt.customLabels,
			}
			...
			expectedCloudAction := "false"
			if tt.cloudAction {
				expectedCloudAction = "true"
			}
			if labels["cloud_action"] != expectedCloudAction {
				t.Errorf("cloud_action = %v, want %v", labels["cloud_action"], expectedCloudAction)
			}
...
func TestBuildLabelsAutoGeneratedPrecedence(t *testing.T) {
	...
	metadata := &tekton.ResourceMetadata{
		DisplayName:   "auto-display",
		ResourceName:  "auto-resource",
		ResourceKind:  "auto-kind",
		EnvUniqueName: "auto-env",
		ClusterID:     "auto-cluster",
		IsCloudAction: false,
...
// TestBuildStepAction tests the BuildKubernetesStepAction function
func TestBuildStepAction(t *testing.T) {
	stepActionName := "setup-credentials-abc123"
	...
	stepAction := tekton.BuildKubernetesStepAction(stepActionName, namespace, labels)
...
func TestExtractMetadataFromObject(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			namespace, name, err := extractMetadataHelper(obj)
...
// extractMetadataHelper is a test helper function to extract namespace and name from an unstructured object
func extractMetadataHelper(obj *unstructured.Unstructured) (string, string, error) {
	...
	if !found || namespace == "" {
		return "", "", fmt.Errorf("namespace not found or empty")
	}
